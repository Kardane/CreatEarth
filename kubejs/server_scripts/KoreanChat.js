...
var 영어타이핑 = "rRseEfaqQtTdwWczxvgkoiOjpuPhynbmlYUIHJKLBNMASDFGZXCV";
var 한글타이핑 = "ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎㅏㅐㅑㅒㅓㅔㅕㅖㅗㅛㅜㅠㅡㅣㅛㅕㅑㅗㅓㅏㅣㅠㅜㅡㅁㄴㅇㄹㅎㅋㅌㅊㅍ";
...
onEvent('command.registry', event => {
  ...
  event.register(
    Commands.literal("krc")
    .executes(ctx => {
      let player = ctx.source.getPlayerOrException().asKJS()
      if(player.tags.contains('krc') == false){
        player.tags.add('krc')
        player.tell(Text.of([Text.lightPurple(`[알림] `), Text.gold(`한글 채팅을 활성화 했습니다. 비활성화 하려면 명령어 /krc를 사용하세요.`)]))
      } else {
        player.tags.remove('krc')
        player.tell(Text.of([Text.lightPurple(`[알림] `), Text.gold(`한글 채팅을 비활성화 했습니다. 활성화 하려면 명령어 /krc를 사용하세요.`)]))
      }
      return 1
    })
  )
...
onEvent('player.chat', function (event) {
  if(event.player.tags.contains('krc') == true){
    let server=event.server
    const msg=event.message;
    const kor=engToKor(msg);
    event.server.runCommandSilent(`tellraw @a [{"text":"<${event.player.getDisplayName().getString()}>","color":"${event.player.getDisplayName().getStyle().color}"},{"text":" ${kor}","color":"white"}]`)
    //event.server.tell(`<` + event.player.getDisplayName().getString() + `> ${kor}`)
    event.cancel()
  }
})

onEvent('player.logged_in', event => {
    event.player.tags.add('krc')
    event.player.tell(Text.of([Text.lightPurple(`[알림] `), Text.gold(`한글 채팅을 활성화 했습니다. 비활성화 하려면 명령어 /krc를 사용하세요.`)]))
})
